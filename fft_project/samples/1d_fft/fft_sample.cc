...
void fft2d(fft_matrix &v, int N, fft_type root) {
  ...
  for (int i = 0; i < n; i++, wi *= root) {
    ...
    for (int j = 0; j < n; j++, wj *= root) {
      v[i][j] = matricies[0][0][i][j] + matricies[1][0][i][j] * wi +
                matricies[0][1][i][j] * wj + matricies[1][1][i][j] * wi * wj;
      v[i + n][j] = matricies[0][0][i][j] - matricies[1][0][i][j] * wi +
                    matricies[0][1][i][j] * wj -
                    matricies[1][1][i][j] * wi * wj;
      v[i][j + n] = matricies[0][0][i][j] + matricies[1][0][i][j] * wi -
                    matricies[0][1][i][j] * wj -
                    matricies[1][1][i][j] * wi * wj;
      v[i + n][j + n] = matricies[0][0][i][j] - matricies[1][0][i][j] * wi -
                        matricies[0][1][i][j] * wj +
                        matricies[1][1][i][j] * wi * wj;
    }
  }
